(function (exports) {
	...
	var calendarGenerators = {
		...
		yahoo: function (event) {
			if (event.allday) {
				...
				var eventDuration = event.tzend ? (event.tzend.getTime() - event.tzstart.getTime()) / MS_IN_MINUTES : event.duration;
		...
		off365: function (event) {
			...
			var href = encodeURI(["https://outlook.office365.com/owa/", "?path=/calendar/action/compose", "&rru=addevent", "&subject=" + (event.title || ""), "&startdt=" + (startTime || ""), "&enddt=" + (endTime || ""), "&body=" + (event.description || ""), "&location=" + (event.address || ""), "&allday=" + event.allday ? "true" : "false"].join(""));
  ...
  --------------- */
	...
	document.addEventListener("DOMContentLoaded", function (event) {
		// console.log(document.querySelectorAll(CONFIG.selector));
